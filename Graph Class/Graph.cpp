	...
	, k_max_range_(max_range)
{
	...
	edge_matrix_ = vector<vector<int>>(num_of_vertices_, vector<int>(num_of_vertices));
...
template<class T>
Graph<T>::Graph(vector<T> vec_node_name, double density, int max_range)
	: num_of_vertices_(0)
	, num_of_edges_(0)
	, k_max_range_(max_range)
{
	// Check if parameters are within legal ranges
	assert((vec_node_name. >= 0) && (density >= 0.0) && (density <= 1.0) && (max_range > 0));
